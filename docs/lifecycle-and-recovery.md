...
If any payout leg uses Venmo, Cash App, or PayPal, the cash-out restricts intent
signaling to the Peer Pay merchant group and enables dispute protection for
that payment method. Signed `cashout()` first proves protection is ready, then
confirms the deposit and both follow-ups with the same viem wallet. For
`prepare()`, finalize the confirmed receipt, iterate
`accessPolicyPaymentMethods` with `prepareAccessPolicy`, and iterate
`disputeProtectionPaymentMethods` with `prepareDisputeProtection`. Any viem
`WalletClient`, including an EOA, can submit them; Privy is not required.
...
`DISPUTE_PROTECTION_UNAVAILABLE` occurs before a deposit exists. If
`DISPUTE_PROTECTION_CONFIGURATION_FAILED` occurs, the deposit already exists;
inspect its recovery hash before preparing the method-scoped protection again.
...
| `DISPUTE_PROTECTION_UNAVAILABLE`          | yes       | No deposit exists; retry after the protected stack is ready or choose an unrestricted rail.                                                  |
| `DISPUTE_PROTECTION_CONFIGURATION_FAILED` | no        | The deposit exists; inspect the protection hash and recover it without repeating the cash-out.                                               |
