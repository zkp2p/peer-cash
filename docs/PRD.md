...
| `prepare(input)`                           | Same as cashout but returns unsigned `txs[]` + same-index `steps[]` - agent wallets, AA, server keys                 |
...
`prepareWithdraw` and `prepareTopUp` mirror the mutating verbs for host-side signing. Prepare results include readable step labels (`approve`, `createDeposit`, `pruneExpiredIntents`, `withdrawDeposit`, `removeFunds`, `addFunds`) in the same order as `txs[]`. Every transaction - including approves - carries ERC-8021 attribution (`peer-cash`, then integrator `referrer` codes, then the Base builder code).
...
- **prepare/execute split** - unsigned `txs[]` for any signer; same-index `steps[]` explain each transaction before policy layers inspect calldata pre-signature.
