...
const builtInCashTools = [
  ...
  },
  {
    name: 'cash_cashout',
    description:
      'Start a Base-USDC cash-out using the custody-separated prepare path. Returns UNSIGNED transactions plus same-index steps [approve, createDeposit]; signing and ordered submission stay host-side. Venmo, Cash App, and PayPal also require prepareAccessPolicy(depositId) after createDeposit confirms. For another source asset, complete cash_source_quote and cash_source_status first, then pass the guaranteed Base USDC output amount here.',
    inputSchema: {
      type: 'object',
      properties: {
        amount: bigintString,
