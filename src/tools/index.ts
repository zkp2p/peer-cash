...
const builtInCashTools = [
  ...
  },
  {
    name: 'cash_cashout',
    description:
      'Start a Base-USDC cash-out using the custody-separated prepare path. Returns UNSIGNED transactions plus same-index steps [approve, createDeposit]; signing and ordered submission stay host-side. Venmo, Cash App, and PayPal require both method-scoped Peer Pay policy and dispute-protection follow-ups after createDeposit confirms. The host must call CashClient.finalizePreparedCashout(receipt), prepare and confirm CashClient.prepareAccessPolicy for every accessPolicyPaymentMethods entry, and prepare and confirm CashClient.prepareDisputeProtection for every disputeProtectionPaymentMethods entry. These receipt/signing methods are not separate built-in tools. For another source asset, complete cash_source_quote and cash_source_status first, then pass the guaranteed Base USDC output amount here.',
    inputSchema: {
      type: 'object',
      properties: {
        amount: bigintString,
