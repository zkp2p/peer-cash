...
export function createApp(options: AppOptions) {
  ...
  publicApp.get('/api/health', (context) => context.json({ status: 'ok' }));
  publicApp.get('/api/report', async (context) => {
    const result = await payments.evm.charge({
      amount: '0.01',
      description: 'Merchant report',
    })(context.req.raw);
    if (result.status === 402) return result.challenge;
    return result.withReceipt(context.json({ report: 'paid MPP resource' }));
  });

  const adminApp = new Hono();
  adminApp.get('/status', (context) => context.json(formatSnapshot(revenue)));
  adminApp.post('/cashout', async (context) => {
    let amount = 0n;
    let reserved = false;
    try {
      const body = await readCashoutBody(context.req.raw);
      ...
      if (amount <= 0n) return context.json({ error: 'cash-out amount must be positive' }, 400);
      ...
      if (isCashError(error)) return context.json({ error: error.toJSON() }, 400);
      if (error instanceof CashoutRequestError) {
        return context.json({ error: error.message }, 400);
      }
      console.error('Unable to prepare Peer cash-out', error);
      return context.json({ error: 'Unable to prepare Peer cash-out' }, 500);
    }
  });
...
async function readCashoutBody(request: Request): Promise<{ amountUsdc?: string }> {
