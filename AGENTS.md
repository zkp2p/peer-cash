...
You are integrating Peer Cash: an offramp that routes any Relay-supported
...
// Optional: live Relay source chains/tokens.
...
- **Do not hardcode Relay source assets.** Use Relay SDK-backed
...
| `SOURCE_ROUTE_UNSUPPORTED_IN_PREPARE` | no        | Use signer-backed `cashout({ source })`, or bridge with Relay first and then `prepare()` Base USDC |
