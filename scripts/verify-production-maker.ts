...
const openDeposits = new Set<string>();
...
async function cleanupOpenDeposits(): Promise<void> {
  for (const depositId of openDeposits) {
    try {
      const order = await waitForOrder(depositId, () => true, 'cleanup indexing');
      if (!order.nextActions.includes('withdraw')) {
        console.error(`  cleanup blocked for ${depositId}: order is ${order.state}`);
        continue;
      }
      const withdrawal = await cash.withdraw(depositId, { signer });
      openDeposits.delete(depositId);
      console.error(
...
async function main(): Promise<void> {
  ...
  openDeposits.add(direct.depositId);
  ...
  openDeposits.delete(direct.depositId);
  ...
  openDeposits.add(routed.depositId);
  ...
  openDeposits.delete(routed.depositId);
